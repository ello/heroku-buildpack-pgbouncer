...
ADMIN_DB_USER=${PGBOUNCER_ADMIN_USERNAME:-`date +%s | md5 | base64 | head -c 32`}
ADMIN_DB_PASS=${PGBOUNCER_ADMIN_PASSWORD:-`date +%s | md5 | base64 | head -c 32`}
n=1
...
admin_users = ${ADMIN_DB_USER}
[databases]
EOFEOF

ADMIN_MD5_PASS="md5"`echo -n ${ADMIN_DB_PASS}${ADMIN_DB_USER} | md5sum | awk '{print $1}'`
cat >> /app/vendor/pgbouncer/users.txt << EOFEOF
"$ADMIN_DB_USER" "$ADMIN_MD5_PASS"
EOFEOF
...
export PGBOUNCER_ADMIN_DATABASE_URL=postgres://$ADMIN_DB_USER:$ADMIN_DB_PASS@localhost:6000/pgbouncer
